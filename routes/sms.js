...
async function pushAlertToArea(alert) {
  const nearby = await getSubscribersByLocation(alert.location);
...
router.post('/incoming', async (req, res) => {
  ...
  try {
    ...
    if (keyword === 'SOS') {
      ...
      await createAlert({ location: "oshodi lagos", category: CATEGORIES.SOS, description: "Active SOS Incident detected. User in distress.", reporterPhone: from, source: 'sms' });

      const subs = await getSubscribersByLocation("lagos"); // simple demo logic
    ...
    if (keyword === 'GUIDE') {
      await updateBehavior(from, 'GUIDE');
      const profile = await getBehaviorProfile(from);
    ...
    if (keyword === 'REPORT') {
      await updateBehavior(from, 'REPORT');
      ...
      const alert = await createAlert({ location, category: CATEGORIES.SCAM, description, reporterPhone: from, source: 'sms' });
      await sendSMS(from, `NaijaSafe: Report received! Alert #${alert.id || alert._id} for "${location}" is now live.\nWe are notifying everyone registered in that area. Thank you for keeping Nigeria safe.`);
    ...
    if (keyword === 'CHECK') {
      ...
      const found = await getAlertsByLocation(location);
      if (found.length === 0) {
    ...
    console.error('[NaijaSafe] /sms/incoming error (server kept alive):', err.message);
    res.sendStatus(200);
  }
});
